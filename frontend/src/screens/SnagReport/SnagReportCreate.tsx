...
interface SnagReportCreateProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const SnagReportCreate: React.FC<SnagReportCreateProps> = ({ open, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<SnagReportRecord>>({});
  const setField = (name: string, value: unknown) => setForm((p) => ({ ...p, [name]: value }));
  const { triggerFlows } = useFlows();

  const mutation = useMutation({
    mutationFn: (values: Partial<SnagReportRecord>) => dataProvider.create('SnagReport', values),
    onSuccess: (_data, values) => { triggerFlows('create', 'SnagReport', values as Record<string, unknown>); onSuccess(); onClose(); setForm({}); },
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Create SnagReport</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  <LookupSelect label="Criticality *" value={String(form.criticality ?? '')} onChange={(v) => setField('criticality', v ? Number(v) : null)} searchable={false} options={[{ label: 'AOG', value: '0' }, { label: 'MEL', value: '1' }, { label: 'Routine', value: '2' }]} />
                </div>
                <div>
                ...
                <div>
                  <LookupSelect label="Status *" value={String(form.status ?? '')} onChange={(v) => setField('status', v ? Number(v) : null)} searchable={false} options={[{ label: 'Open', value: '0' }, { label: 'InProgress', value: '1' }, { label: 'PendingCRS', value: '2' }, { label: 'Closed', value: '3' }, { label: 'Cancelled', value: '4' }]} />
                </div>
                <div>
