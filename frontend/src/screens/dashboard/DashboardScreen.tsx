...
export const DashboardScreen: React.FC = () => {
  ...
  React.useEffect(() => {
    const sources = [
      ...
      { name: 'Snag / Uygunsuzluk Raporu', url: `${API_BASE}/api/services/app/SnagReport/GetAll?MaxResultCount=1`, path: 'result.totalCount' },
    ];
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 16 }}>
        ...
        <div style={{ gridColumn: 'span 4' }}>
          ...
          <TkCard>
            <div slot="content" style={{ padding: 20 }}>
              <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Total Snag / Uygunsuzluk Raporu</div>
        ...
        <div style={{ gridColumn: 'span 6' }}>
          <div style={{ background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #e8e8e8' }}>
            {api_block_7Loading ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>ID</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>First Name</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Last Name</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Badge Number</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Role</th></tr></thead>
                <tbody>
                  {(Array.isArray(api_block_7Data) ? api_block_7Data : extractArray(api_block_7Data)).map((row: Record<string, unknown>, i: number) => (
                    <tr key={i}><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['id'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['firstName'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['lastName'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['badgeNumber'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['role'] ?? '')}</td></tr>
                  ))}
                </tbody>
              </table>
            )}
