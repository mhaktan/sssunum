        ...
        ISnagReportAppService
    {
        ...
        protected override IQueryable<SnagReport> CreateFilteredQuery(PagedSnagReportResultRequestDto input)
        {
            return Repository.GetAll()
                ...
                .WhereIf(input.Criticality.HasValue, x => x.Criticality == (Criticality)input.Criticality.Value)
        ...
        public async Task<SnagReportDto> ChangeStatusAsync(long id, ChangeStatusInput input)
        {
            ...
            ("Open", "Cancelled", "Cancel", true),
            ("InProgress", "Cancelled", "Cancel", true)
            };
            ...
            if (input.Action == "Submit" && (input.ActionData == null || !input.ActionData.ContainsKey("actionTaken") || string.IsNullOrWhiteSpace(input.ActionData["actionTaken"]) || !input.ActionData.ContainsKey("certifyingStaffId") || string.IsNullOrWhiteSpace(input.ActionData["certifyingStaffId"])))
                throw new Abp.UI.UserFriendlyException("Submit requires: actionTaken, certifyingStaffId");
